...
export default function BracketViewPage() {
  ...
  const getRoundName = (roundIndex: number, totalRounds: number) => {
    const matchupsInRound = bracketState.rounds[roundIndex]?.length || 0;
            ...
            {bracketState.champion && (
              <div className="mt-8 text-center">
                <div className="bg-gradient-to-r from-primary to-secondary text-white rounded-2xl p-6">
                  ...
                  <p className="text-2xl font-fredoka font-bold">{bracketState.champion.title}</p>
                  {bracketState.champion.releaseYear && (
                    <p className="text-sm font-nunito mt-1 opacity-90">{bracketState.champion.releaseYear}</p>
                  )}
                </div>
              </div>
            )}
